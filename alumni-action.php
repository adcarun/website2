<?php
	...
	include("./phpincludes/alumni-smtp-details.php");
	...
	if(isset($_POST['AlumniName']) && $_POST['AlumniName'] != "")
	{	
		if(!preg_match("/[A-Za-z]/", $_POST['AlumniName']))
		{
		...
		if(!preg_match("/^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/", $_POST['AlumniEmail']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		if(!preg_match("/^[0-9]*$/", $_POST['AlumniContact']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		/*if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response']))
		{
			...
			{*/
				...
				$fp=fopen("./mailer/AlumniMailer.html","r");
				$message= fread($fp,filesize("./mailer/AlumniMailer.html"));
				
				$DateTime = convertDBDateTime(date("Y-m-d H:i:s"));
				$Subject = "Alumni Feedback posted by ".$_POST['name']. " on ".$DateTime;
				...
				$message=str_replace('$AlumniName', $_POST['AlumniName'],$message);
				$message=str_replace('$AlumniEmail',$_POST['AlumniEmail'],$message);
				$message=str_replace('$AlumniContact',$_POST['AlumniContact'],$message);
				$message=str_replace('$SelectType',$_POST['SelectType'],$message);
				
				$message=str_replace('$AlumniProgramName',$_POST['AlumniProgramName'],$message);
				$message=str_replace('$AlumniBatch',$_POST['AlumniBatch'],$message);
				$message=str_replace('$AlumniFeedback',$_POST['AlumniFeedback'],$message);

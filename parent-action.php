<?php
	...
	if(isset($_POST['ParentName']) && $_POST['ParentName'] != "")
	{	
		if(!preg_match("/[A-Za-z]/", $_POST['ParentName']))
		{
		...
		if(!preg_match("/^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/", $_POST['ParentEmail']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		if(!preg_match("/^[0-9]*$/", $_POST['ParentContact']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		/*if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response']))
		{
			...
			{*/
				...
				$fp=fopen("./mailer/ParentMailer.html","r");
				$message= fread($fp,filesize("./mailer/ParentMailer.html"));
				
				$DateTime = convertDBDateTime(date("Y-m-d H:i:s"));
				$Subject = "Parent Feedback posted by ".$_POST['name']. " on ".$DateTime;
				...
				$message=str_replace('$ParentName', $_POST['ParentName'],$message);
				$message=str_replace('$ParentEmail',$_POST['ParentEmail'],$message);
				$message=str_replace('$ParentContact',$_POST['ParentContact'],$message);
				$message=str_replace('$SelectType',$_POST['SelectType'],$message);
				
				$message=str_replace('$ChildrenProgram',$_POST['ChildrenProgram'],$message);
				$message=str_replace('$ChildrenBranch',$_POST['ChildrenBranch'],$message);
				$message=str_replace('$ParentFeedback',$_POST['ParentFeedback'],$message);

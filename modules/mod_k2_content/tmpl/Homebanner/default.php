...
<div id="home-carousel" class="home-carousel carousel slide" data-ride="carousel">
	  ...
	  <?php $cntInd=0; foreach ($items as $key=>$item):	?>
        <li data-target="#home-carousel" data-slide-to="<?php echo $cntInd; ?>" <?php if($cntInd==0){ ?> class="active" <?php } ?>></li>
	  <?php $cntInd++; endforeach; ?>		        
      </ol>
    
      <!-- Wrapper for slides -->
      <div class="carousel-inner" role="listbox">
	  <?php $cnt=0; foreach ($items as $key=>$item):	?>
        <div class="item <?php if($cnt==0){ ?>active<?php } ?>">
          <div class="fill" alt="banner" title="banner" style="background-image:url('<?php echo $item->image; ?>');"></div>
          <div class="carousel-caption">
		  <?php $vals=json_decode($item->extra_fields); //print_r($vals); ?>
            <?php if($vals[0]->value !=""){ ?><h3><?php echo $vals[0]->value; ?></h3><?php } ?>
			<?php if($vals[1]->value !=""){ ?><h4><?php echo $vals[1]->value; ?></h4><?php } ?>
            <?php if(($vals[2]->value[0] !="") && ($vals[2]->value[1] != "")){ ?>
			<p>
            	<a href="<?php echo $vals[2]->value[1]; ?>" target="<?php if($vals[2]->value[2]=="same"){ ?>_parent<?php }else{ ?>_blank<?php } ?>"><?php echo $vals[2]->value[0]; ?> <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span></a>
            </p>
			<?php } ?>
          </div>
        </div>

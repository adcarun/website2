<?php
	...
	if(isset($_POST['FacultyName']) && $_POST['FacultyName'] != "")
	{	
		if(!preg_match("/[A-Za-z]/", $_POST['FacultyName']))
		{
		...
		if(!preg_match("/^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/", $_POST['FacultyEmail']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		if(!preg_match("/^[0-9]*$/", $_POST['FacultyContact']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		/*if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response']))
		{
			...
			{*/
				...
				$fp=fopen("./mailer/FacultyMailer.html","r");
				$message= fread($fp,filesize("./mailer/FacultyMailer.html"));
				
				$DateTime = convertDBDateTime(date("Y-m-d H:i:s"));
				$Subject = "Faculty Feedback posted by ".$_POST['name']. " on ".$DateTime;
				...
				$message=str_replace('$FacultyName', $_POST['FacultyName'],$message);
				$message=str_replace('$FacultyEmail',$_POST['FacultyEmail'],$message);
				$message=str_replace('$FacultyContact',$_POST['FacultyContact'],$message);
				$message=str_replace('$SelectType',$_POST['SelectType'],$message);
				
				$message=str_replace('$FacultyFeedback',$_POST['FacultyFeedback'],$message);

<?php
	...
	if(isset($_POST['RecruitersName']) && $_POST['RecruitersName'] != "")
	{	
		if(!preg_match("/[A-Za-z]/", $_POST['RecruitersName']))
		{
		...
		if(!preg_match("/^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/", $_POST['RecruitersEmail']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		if(!preg_match("/^[0-9]*$/", $_POST['RecruitersContact']))
		{
			$_SESSION['Error'] = "Invalid Data";
			$_SESSION['Post'] = $_POST;
			header("Location:".$_SERVER['HTTP_REFERER']);
		...
		/*if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response']))
		{
			...
			{*/
				...
				$fp=fopen("./mailer/RecruitersMailer.html","r");
				$message= fread($fp,filesize("./mailer/RecruitersMailer.html"));
				
				$DateTime = convertDBDateTime(date("Y-m-d H:i:s"));
				$Subject = "Recruiters Feedback posted by ".$_POST['name']. " on ".$DateTime;
				...
				$message=str_replace('$RecruitersName', $_POST['RecruitersName'],$message);
				$message=str_replace('$RecruitersEmail',$_POST['RecruitersEmail'],$message);
				$message=str_replace('$RecruitersContact',$_POST['RecruitersContact'],$message);
				$message=str_replace('$SelectType',$_POST['SelectType'],$message);
				
				$message=str_replace('$RecruitersOrganisation',$_POST['RecruitersOrganisation'],$message);
				$message=str_replace('$DesignationOrganisation',$_POST['DesignationOrganisation'],$message);
				$message=str_replace('$RecruitersFeedback',$_POST['RecruitersFeedback'],$message);
